...
$headers = @{
    ...
    'ClientID' = 'CW-Registered-Client-ID'
}

$body = @{
    "UserName" = 'Automate-Integration-Username'
    "Password" = 'Automate-Integration-Password'
}

$body = $body | ConvertTo-Json
$counter = 1
write-host "Connecting to AD to refresh Computer List"
$ad_Computers = (Get-ADComputer -Filter * -SearchBase "CN=Computers,DC=domainname,DC=local").Name
$ad_Computers = Get-ADComputer -Filter "*" -Properties *
    $ad_Computers = $ad_Computers | ?{$_.LastLogonDate -ge (Get-Date).AddDays(-1)}
write-host "Found: "$ad_computers.count " AD Computers"



    write-host "Connecting to Automate to refresh List"
    $bearer = Invoke-RestMethod -Method POST -uri "https://domain.hostedrmm.com/cwa/api/v1/apitoken" -Body $body -Headers $headers -ContentType $headers.'Content-type'
    ...
    do{
        $computers = Invoke-RestMethod -Method GET -uri "https://domain.hostedrmm.com/cwa/api/v1/Computers?pageSize=1000&page=$($count)" -Headers $request_headers -ContentType 'application/json'
        $all_computers += $computers
        $count++
    }while($computers.count -eq 0)
    
    $no_heartbeat = $all_computers | ? {($_.lastheartbeat -eq $null)}
    $with_heartbeat = $all_computers | ? {($_.lastheartbeat -ne $null)}

    #$computers_to_start_service = $($with_heartbeat | ? {(Get-Date $_.lastheartbeat) -gt (Get-Date).AddDays(-2)})
    write-host "Found: "$all_computers.count " Automate Computers"
    $computers_to_start_service = $ad_computers | ? {$_.name -notin $all_computers.ComputerName}
    $counter = $computers_to_start_service.count
    write-host "Found: "$computers_to_start_service.count "Computers to Attempt Install" -ForegroundColor Green

    foreach($computer in $computers_to_start_service){
            
            $comp = $computer.Name
            write-host "Testing Service on $comp" $computers_to_start_service.Indexof($computer)"/"($computers_to_start_service.count-1)


		    #Test WinRM, if this fails we can't do shit
		    Write-Host "Testing WSMAN Connection to $comp on $($computer.IPv4Address)" -ForegroundColor DarkGray 
		    $Heartbeat = (Test-WSMan -ComputerName $comp -ErrorAction SilentlyContinue)
		    If (!$Heartbeat)
		    {
			    "WinRM appears to be off for $comp" | Out-File $LogFile\log3.txt -Append -Force
			    Write-Host "$comp is not able to be connected to via WinRM" -ForegroundColor Gray
    ...
    for ($x=$ip1; $x -le $ip2; $x++)
    {
        ...
        If($Exclude -notcontains $ip[3])
        {
            $ip -join '.'
                    ...
                    Install-Automate -Server 'domain.hostedrmm.com' -Token e4b51aa47ec41dea8b3afe23478a019c
